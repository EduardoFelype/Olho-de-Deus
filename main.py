...
def _clean_json(obj):
    """Remove emojis e caracteres fora do BMP recursivamente para garantir JSON válido no Windows."""
    if isinstance(obj, dict):
        return {k: _clean_json(v) for k, v in obj.items()}
    if isinstance(obj, list):
        return [_clean_json(i) for i in obj]
    if isinstance(obj, str):
        # Remove caracteres fora do plano BMP (emojis U+10000+)
        return re.sub(r'[\U00010000-\U0010FFFF]', '', obj)
    return obj
...
def save_results(full_results: dict, target: str):
    ...
    json_file = os.path.join(report_dir, f"{base}_report.json")
    pdf_file  = os.path.join(report_dir, f"{base}_report.pdf")
    html_file = os.path.join(report_dir, f"{base}_dashboard.html")

    with open(json_file, "w", encoding="utf-8") as f:
        json.dump(_clean_json(full_results), f, indent=2, ensure_ascii=False, default=str)
...
def main():
    ...
    if "secrets"      not in full: full["secrets"]      = []
    if "technologies" not in full: full["technologies"] = []
    if "ai_insights"  not in full: full["ai_insights"]  = []
    if "ai_top_issues" not in full: full["ai_top_issues"] = []
    if "cve_matches"  not in full: full["cve_matches"]  = []
    ...
    if full.get("ai_insights"):
        ...
        for ins in full["ai_insights"]:
            # Remove emojis do terminal também (Windows cmd não suporta)
            print_status(re.sub(r'[\U00010000-\U0010FFFF]', '', str(ins)), "WARN")
