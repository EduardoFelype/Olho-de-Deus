    ...
    def crawl(self):
        ...
        while self._queue and len(pages) < self.max_pages:
            ...
            if "html" in r.headers.get("Content-Type",""):
                ...
                # Links internos + extração de params reais
                for tag in soup.find_all("a", href=True):
                    full = urljoin(url, tag["href"])
                    if self._is_internal(full) and full not in self._visited:
                        self._queue.append(full)
                        # Extrai params da query string do link
                        parsed_link = urlparse(full)
                        if parsed_link.query:
                            found_params = list(parse_qs(parsed_link.query).keys())
                            if found_params and self.context:
                                self.context.add_url_params(full, found_params)
